...
private struct VideoDetailSmoothHeaderSyncState: Equatable {
    ...
    let inactiveNormalizedOffsetY: CGFloat
    ...
    static func state(
        ...
        if activeOffset < resolvedCollapseDistance {
            ...
            return VideoDetailSmoothHeaderSyncState(
                ...
                inactiveNormalizedOffsetY: scrollingOffset
            )
        }
        return VideoDetailSmoothHeaderSyncState(
            headerContainerY: -resolvedCollapseDistance,
            inactiveNormalizedOffsetY: resolvedCollapseDistance
        )
    }
}
...
private final class VideoDetailVerticalScrollPageViewController: UIViewController {
    ...
    @discardableResult
    func syncHeaderOffsetFromActivePage(_ normalizedOffsetY: CGFloat) -> Bool {
        loadViewIfNeeded()
        guard !listScrollView.isTracking, !listScrollView.isDragging, !listScrollView.isDecelerating else {
            return false
        }
        return setNormalizedContentOffsetYIfReachable(normalizedOffsetY)
    }
...
private struct VideoDetailTabPager: UIViewControllerRepresentable {
    ...
    final class Coordinator: NSObject, UIScrollViewDelegate {
        ...
        func shouldBeginHorizontalPagingPan(
            ...
            guard !view.hasActiveVerticalScrollDescendant(at: startLocation, excluding: view) else {
                return false
            }
    ...
    final class PagingViewController: UIViewController {
        ...
        private func syncInactivePageHeaderOffset(activeOffset providedActiveOffset: CGFloat? = nil) {
            ...
            for tab in VideoPageTab.allCases where tab.pageIndex != pagerPosition.selectedIndex {
                verticalPageController(for: tab)?.syncHeaderOffsetFromActivePage(nextSyncState.inactiveNormalizedOffsetY)
            }
        }
...
private extension UIView {
    func hasActiveVerticalScrollDescendant(at location: CGPoint, excluding excludedView: UIView) -> Bool {
        ...
        while let view = current, view !== excludedView {
            if let listScrollView = view as? UIScrollView,
               ...
               listScrollView.contentSize.height > listScrollView.bounds.height + 1,
               listScrollView.contentSize.height >= listScrollView.contentSize.width,
               listScrollView.isTracking || listScrollView.isDragging || listScrollView.isDecelerating {
                return true
            }
