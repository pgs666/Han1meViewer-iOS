...
| 问题 | 现状 | 说明 |
| --- | --- | --- |
| 首页大标题滚动收缩为自绘实现 | 已缓解 | 为避免侧滑返回时「首页」标题闪现在播放器上层，首页隐藏系统导航栏并自绘标题。视觉接近系统大标题，但收缩曲线与 Dynamic Type 自适应不完全一致，待未来 UIKit 导航重构后还原（详见 `docs/known-issues/`）。 |
...
> 如发现此处未列出的问题，欢迎按上方「问题反馈」提交 Issue。
